...
# Files setup
test_score_filename = 'results/new_method_v3/test_scores.csv'
...
model_files_dir_name = 'model/new_method_v3/'
...
for i in range(aeu.repeat_number):
    ...
    mv3 = aeu.BuildOurMethodV3(original_dim=train_data.shape[1],
                               hidden_units=best_layer_type,
                               model_dir_path=saved_ae_network_path,
                               iteration_number=i)

    mv3.train(train_data=train_data,
              save_model=True,
              print_and_plot_history=True,
              show_plots=False)

    n_loss, n_acc = mv3.test_model(test_data=train_data)
    v_loss, v_acc = mv3.test_model(test_data=validation_data)
    t_loss, t_acc = mv3.test_model(test_data=abnormal_data, is_abnormal=True)
...
output_string = 'Global summary of abnormal trajectory detection with our new method v.3\n'
output_string += '-----------------------------------------------------------------------\n'
