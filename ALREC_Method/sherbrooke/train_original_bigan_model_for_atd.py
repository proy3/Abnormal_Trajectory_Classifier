...
# Files setup
test_score_filename = 'results/original_bigan/test_scores.csv'
...
model_files_dir_name = 'model/original_bigan/'
...
for i in range(aeu.repeat_number):
    ...
    aae = aeu.BuildOriginalBiGAN(original_dim=train_data.shape[1],
    ...
    output_string = 'Iteration {}: ae_train_score = {}; ae_validation_score = {}'\
        .format(i, ae_train_score, ae_validate_score)
...
output_string = 'Global summary of abnormal trajectory detection with traditional bidirectional GAN\n'
output_string += '----------------------------------------------------------------------------------\n'
output_string += 'On average,\n'
output_string += '\t{0:.2f}% of normal training samples are detected as normal;\n'.format(np.mean(
    normal_train_ratio_list))
output_string += '\t{0:.2f}% of normal validation samples are detected as normal;\n'.format(np.mean(
    normal_valid_ratio_list))
