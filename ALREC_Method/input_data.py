...
def _rearrange_data(object_id, object_label, x, y, v_x, v_y, generate_graph, trajectory_image):
    ...
    :return:
    """
    if sub_trajectory_size > x.shape[0]:
        # Resize all the input array to the expected size
        zoom_ratio = sub_trajectory_size / float(x.shape[0])
        x = zoom(x, (zoom_ratio, 1))
        y = zoom(y, (zoom_ratio, 1))
        v_x = zoom(v_x, (zoom_ratio, 1))
        v_y = zoom(v_y, (zoom_ratio, 1))

    expected_input_size = sub_trajectory_size + \
                          int(np.ceil(float(x.shape[0] - sub_trajectory_size) / trajectory_stride)) * trajectory_stride
    ...
    if generate_graph:
        for i in range(x.shape[0] / sub_trajectory_size):
            trajectory_image.add_trajectory(x[i*sub_trajectory_size:(i+1)*sub_trajectory_size].flatten(),
                                            ...
                                            line_width=5,
                                            line_color=['b','g','purple'][object_label])
...
def _rearrange_data_v2(object_id, object_label, x, y, v_x, v_y, bb_h, bb_w, ori,
                       generate_graph, trajectory_image):
    """
    Generates an array formed by the input data in the following format:
    [[Object_id,x_0,y_0,v_x_0,v_y_0,bb_h_0,bb_w_0,ori_0,...,x_31,y_31,v_x_31,v_y_31,bb_h_31,bb_w_31,ori_31]
     [Object_id,x_32,y_32,v_x_32,v_y_32,bb_h_32,bb_w_32,ori_32,...,x_61,y_61,v_x_61,v_y_61,bb_h_61,bb_w_61,ori_61]
     ...]
    :param object_id:
    :param x:
    :param y:
    ...
    :return:
    """
    if sub_trajectory_size_v2 > x.shape[0]:
        # Resize all the input array to the expected size
        zoom_ratio = sub_trajectory_size_v2 / float(x.shape[0])
        x = zoom(x, (zoom_ratio, 1))
        y = zoom(y, (zoom_ratio, 1))
        v_x = zoom(v_x, (zoom_ratio, 1))
        v_y = zoom(v_y, (zoom_ratio, 1))
        bb_h = zoom(bb_h, (zoom_ratio, 1))
        bb_w = zoom(bb_w, (zoom_ratio, 1))
        ori = zoom(ori, (zoom_ratio, 1))

    expected_input_size = sub_trajectory_size_v2 + int(np.ceil(float(x.shape[0] - sub_trajectory_size_v2) /
                                                               trajectory_stride)) * trajectory_stride

    rows_number = (expected_input_size - sub_trajectory_size_v2) / trajectory_stride + 1
    ...
    if generate_graph:
        for i in range(x.shape[0] / sub_trajectory_size_v2):
            trajectory_image.add_trajectory(x[i*sub_trajectory_size_v2:(i+1)*sub_trajectory_size_v2].flatten(),
                                            y[i*sub_trajectory_size_v2:(i+1)*sub_trajectory_size_v2].flatten(),
                                            line_width=5,
                                            line_color=['b', 'g', 'purple'][object_label])

    for i in range(sub_trajectory_size_v2):
        array_data = np.hstack((array_data,
                                x[i::trajectory_stride][:rows_number, :],
                                y[i::trajectory_stride][:rows_number, :],
                                v_x[i::trajectory_stride][:rows_number, :],
                                v_y[i::trajectory_stride][:rows_number, :],
                                bb_h[i::trajectory_stride][:rows_number, :],
                                bb_w[i::trajectory_stride][:rows_number, :],
                                ori[i::trajectory_stride][:rows_number, :]))
