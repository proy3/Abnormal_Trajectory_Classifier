...
# Files setup
test_score_filename = 'results/deep_vae/test_scores.csv'
...
model_files_dir_name = 'model/deep_vae/'
...
for i in range(aeu.repeat_number):
    ...
    vae = aeu.BuildSimpleVAE(original_dim=train_data.shape[1],
                             hidden_units=best_layer_type,
                             model_dir_path=saved_ae_network_path)

    vae.train(train_data=train_data,
              save_model=True,
              iteration_number=i,
    ...
    # Get the scores
    ae_train_score = vae.global_mse
    ae_train_scores = vae.mse_per_sample

    ae_validate_score, ae_validate_scores = vae.test_model(test_data=validation_data,
                                                           iteration_number=i)

    ae_tests_score, ae_tests_scores = vae.test_model(test_data=abnormal_data,
                                                     iteration_number=i)
