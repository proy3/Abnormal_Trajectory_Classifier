...
normal_trajectory_data_filename = cdata.extract_augment_and_export_data(is_normal=True,
                                                                        generate_graph=True,
                                                                        show_graph=False)
normal_data = np.genfromtxt(os.path.join(data.dir_name, normal_trajectory_data_filename), delimiter=',')
# Ignore first column representing object_id
normal_data = normal_data[:, 1:]

abnormal_trajectory_data_filename = cdata.extract_augment_and_export_data(is_normal=False,
                                                                          generate_graph=True,
                                                                          show_graph=False)
abnormal_data = np.genfromtxt(os.path.join(data.dir_name, abnormal_trajectory_data_filename), delimiter=',')
# Ignore first column representing object_id
abnormal_data = abnormal_data[:, 1:]

# Best layer type tested in main_test.py
# ref.: https://deeplearning4j.org/deepautoencoder
best_layer_type = (256, 128, 64, 32, 16, 8)

# Files setup
test_score_filename = 'results/new_method_v4_5_3/test_scores.csv'
...
model_files_dir_name = 'model/new_method_v4_5_3/'
...
for i in range(aeu.repeat_number):
    ...
    train_data, validation_data, random_shuffle_seed = data.split_dataset_uniformly(normal_data)

    # The trained model will be saved
    saved_ae_network_path = os.path.join(data.dir_name, model_files_dir_name)

    mv4 = aeu.BuildOurMethodV4v2(original_dim=train_data.shape[1],
